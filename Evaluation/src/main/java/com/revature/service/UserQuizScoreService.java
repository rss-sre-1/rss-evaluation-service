...
public class UserQuizScoreService {
	...
	public UserQuizScore InsertUserQuizScore(UserQuizScore uqs) {
		uqs.setQuiz(qr.findById(uqs.getQuizId()).orElseThrow(() -> new EvaluationException("Unable to find Quiz by id"))); 
		try {
	...
	public List<Long> getTakenQuiz(String userEmail){
		...
		uqsList=uqsr.findQuizByUserEmail(userEmail).orElseThrow(() -> new EvaluationException("Unable to find User Quiz Scores by User Email"));
	...
	public List<UserQuizScore> findByUserAndQuiz(String userEmail, long quizId){
		Quiz quiz = qr.findById(quizId).orElseThrow(() -> new EvaluationException("Unable to find Quiz by id"));
		return uqsr.findByUserEmailAndQuiz(userEmail, quiz).orElseThrow(() -> new EvaluationException("Unable to find User Quiz Scores by User Email"));
	}
	
	public UserQuizScore findById(long id) {
		return uqsr.findById(id).orElseThrow(() -> new EvaluationException("Unable to find User Quiz Score by id"));
	}
}
